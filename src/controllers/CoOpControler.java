...
public class CoOpControler {
	...
	private CoOP currentLevel;
	
	public CoOP getLevel() {
		return currentLevel;
	}
	
	public void start() {
		currentLevel = new CoOP();
		timer.start();
	}
	...
	public void update() {
		...
		if(currentLevel.getEnemies().size() == 0) {
			...
			CoOP newLevel = new CoOP(currentLevel.getShields(),currentLevel.getScore(),currentLevel.getLives());
